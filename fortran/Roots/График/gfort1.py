...
#os.chdir(r"/home/evgeny/Dropbox/Проги/fortran/Roots/")
...
# корректирровка границ диапазона изменения переменной x
...
# корректирровка границ диапазона изменения переменной y
...
# учет нормализации в надписи x-оси
if ixml != 0 :
    if ixml == 1 :
        xlbl += r"$*10$"
    else :
...
while iln < nlast :
    ...
    if line[0][:4] == chdata or iln == nlast : # строка-разделитель или последняя строка
        ...
        x = np.asarray(xlst) # преобразование списка абсцисс в массив
